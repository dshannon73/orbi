...
router.get('/', async (req, res) => {
  try {
    ...
    const { search, ownerId, limit = 50, offset = 0, ownerRolePattern, ownerName, justMyData, currentUserId } = req.query as Record<string, string>;
    ...
    const result = await conn.query(
      `SELECT ${FIELDS} FROM Account WHERE ${clauses.join(' AND ')} ORDER BY LastModifiedDate DESC LIMIT ${limit} OFFSET ${offset}`
    );
    res.json(result);
  } catch (err: any) {
