...
import { postToSlack, createCanvas } from '../services/slack';
...
router.post('/canvas/activities', async (req, res) => {
  try {
    ...
    const baseClauses = ownershipClauses({ ownerRolePattern, ownerName, justMyData, currentUserId });
    ...
    const [events, tasks] = await Promise.all([
      conn.query(
        `SELECT Id, Subject, StartDateTime, EndDateTime, DurationInMinutes, SE_Task_Type__c, Meeting_Type__c,
      ...
      conn.query(
        `SELECT Id, Subject, ActivityDate, SE_Task_Type__c, Status,
    ...
    const dataJson = JSON.stringify({
      ...
      period: periodLabel,
      generatedAt: now,
      events: (events.records as any[]).map(e => ({
        ...
        owner: (e as any)['Owner.Name'] ?? e.Owner?.Name ?? e.OwnerId,
        account: (e as any)['Account.Name'] ?? e.Account?.Name,
        relatedTo: (e as any)['What.Name'] ?? e.What?.Name,
      })),
      tasks: (tasks.records as any[]).map(t => ({
        date: t.ActivityDate,
        subject: t.Subject,
        taskType: t.SE_Task_Type__c,
        status: t.Status,
        owner: t.Owner?.Name ?? t.OwnerId,
        relatedTo: (t as any)['What.Name'] ?? t.What?.Name,
      })),
    }, null, 2);

    const totalEvents = (events.records as any[]).length;
    const totalTasks = (tasks.records as any[]).length;
    const totalHours = Math.round((events.records as any[]).reduce((s, e) => s + (e.DurationInMinutes ?? 0), 0) / 60 * 10) / 10;
...
Always use hours as the primary metric alongside counts. Return only the canvas URL after creating it.`;
    ...
    const url = await createCanvas(title, prompt);
    res.json({ url });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
router.post('/canvas/dcs', async (req, res) => {
  try {
    ...
    if (dateFrom) dcClauses.push(`Opportunity_Close_Date__c >= ${dateFrom}`);
    if (dateTo)   dcClauses.push(`Opportunity_Close_Date__c <= ${dateTo}`);
    const where = dcClauses.length ? `WHERE ${dcClauses.join(' AND ')}` : '';

    const dcs = await conn.query(
      `SELECT Opportunity__r.Name, Opportunity__r.StageName, Opportunity__r.ForecastCategoryName,
    ...
    const records = dcs.records as any[];
    const now = new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
    const periodLabel = dateFrom && dateTo ? `${dateFrom} to ${dateTo}` : now;
    ...
    const dataJson = JSON.stringify({
      ...
      dealContributions: records.map(r => ({
        ...
        account: r.Opportunity__r?.Account?.Name ?? (r.Opportunity__r as any)?.['Account.Name'],
    ...
    const records2 = records as any[];
    const totalPipeline = records2.reduce((s: number, r: any) => s + (r.Opportunity_Amount__c ?? 0), 0);
    const totalSplit = records2.reduce((s: number, r: any) => s + (r.Split_Amount__c ?? 0), 0);
