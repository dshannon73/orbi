...
import { ownershipClauses, picklistToSoqlClause, patternToSoqlClause } from '../lib/filters';
...
router.get('/', async (req, res) => {
  try {
    ...
    const {
      ownerId, type,
      status, priority, taskType, meetingType,
      recordTypeId,
      subject, relatedTo,
      dateFrom, dateTo,
      limit = 50, offset = 0,
    ...
    if (ownerId) {
      ...
      const nameClause = patternToSoqlClause(ownerName.trim(), 'Name');
      if (nameClause) ownerFilter = ` AND OwnerId IN (SELECT Id FROM User WHERE ${nameClause})`;
    } else if (ownerRolePattern && ownerRolePattern.trim()) {
      const roleClause = patternToSoqlClause(ownerRolePattern.trim(), 'UserRole.Name');
      if (roleClause) ownerFilter = ` AND OwnerId IN (SELECT Id FROM User WHERE ${roleClause})`;
    }

    // picklist filters
    const statusClause    = status    ? ' AND ' + picklistToSoqlClause(status,    'Status')            : '';
    const priorityClause  = priority  ? ' AND ' + picklistToSoqlClause(priority,  'Priority')          : '';
    const taskTypeClause  = taskType  ? ' AND ' + picklistToSoqlClause(taskType,  'SE_Task_Type__c')   : '';
    const meetingTypeClause = meetingType ? ' AND ' + picklistToSoqlClause(meetingType, 'Meeting_Type__c') : '';
    ...
    const subjectClause   = subject   ? ' AND ' + patternToSoqlClause(subject,  'Subject')             : '';
    ...
    function soqlDate(d: string, suffix: string) {
      return d.includes('_N_DAYS') ? ` AND ${suffix} >= ${d}` : ` AND ${suffix} >= ${d}T00:00:00Z`;
    }
    function soqlDateTo(d: string, suffix: string) {
...
router.get('/search-related', async (req, res) => {
  try {
    const conn = getConnection();
    const { q = '', oppName = '', accountName = '', ownerName = '', ownerRole = '', currentUserId = '' } = req.query as Record<string, string>;

    // Support both legacy params and unified q
    const term = q || oppName || accountName;
    if (term.length < 2 && ownerName.length < 2 && ownerRole.length < 2) return res.json({ results: [] });

    const esc = (s: string) => s.replace(/'/g, "''");
    const safeTerm = esc(term);

    const oppClauses = ['IsDeleted = false', 'IsClosed = false', `ForecastCategoryName != 'Omitted'`];
    if (term.length >= 2) oppClauses.push(`(Name LIKE '%${safeTerm}%' OR Account.Name LIKE '%${safeTerm}%')`);
    if (ownerName.length >= 2) oppClauses.push(`Owner.Name LIKE '%${esc(ownerName)}%'`);
    if (ownerRole.length >= 2) oppClauses.push(`Owner.UserRole.Name LIKE '%${esc(ownerRole)}%'`);

    const acctClauses = ['IsDeleted = false', `Name LIKE '%${safeTerm}%'`];

    const [oppsRes, acctRes] = await Promise.all([
      conn.query(
        `SELECT Id, Name, ForecastCategoryName, Amount, StageName, CloseDate, LastModifiedDate, LastActivityDate,
      ...
      term.length >= 2 ? conn.query(
        `SELECT Id, Name, LastModifiedDate, Owner.Name, Owner.UserRole.Name FROM Account WHERE ${acctClauses.join(' AND ')}
         ORDER BY LastModifiedDate DESC NULLS LAST LIMIT 10`
      ) : Promise.resolve({ records: [] }),
    ]);

    // Fetch my DCs and recent activities on these opps for ranking
    const oppIds = (oppsRes.records as any[]).map((r: any) => r.Id);
    let myDCOppIds = new Set<string>();
    let activityCountByOpp = new Map<string, number>();

    if (oppIds.length > 0 && currentUserId) {
      const idList = oppIds.map((id: string) => `'${id}'`).join(',');
      ...
      ]).catch(() => [{ records: [] }, { records: [] }]) as any;
      (dcRes.records as any[]).forEach((d: any) => myDCOppIds.add(d.Opportunity__c));
      (actRes.records as any[]).forEach((a: any) => activityCountByOpp.set(a.WhatId, Number(a.cnt ?? 0)));
    }

    const FC_RANK: Record<string, number> = { Commit: 4, 'Best Case': 3, Pipeline: 2, Omitted: 0 };

    const rankedOpps = (oppsRes.records as any[]).map((r: any) => {
      let score = 0;
      ...
      if (daysSinceModified <= 14) score += 15;
      else if (daysSinceModified <= 30) score += 10;
      else if (daysSinceModified <= 60) score += 5;
      if (myDCOppIds.has(r.Id)) score += 25;
    ...
    const accounts = (acctRes.records as any[]).map((r: any) => ({
...
router.get('/record-types', async (req, res) => {
  try {
    ...
    const result = await conn.query(
      `SELECT Id, Name FROM RecordType WHERE SobjectType = 'Event' AND IsActive = true ORDER BY Name ASC`
    );
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const [task, event] = await Promise.all([
      conn.query(`SELECT ${TASK_DETAIL} FROM Task WHERE Id = '${req.params.id}'`).catch(() => ({ records: [] as any[], done: true, totalSize: 0 })),
      conn.query(`SELECT ${EVENT_DETAIL} FROM Event WHERE Id = '${req.params.id}'`).catch(() => ({ records: [] as any[], done: true, totalSize: 0 })),
    ]);
