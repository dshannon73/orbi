...
exports.patternToSoqlClause = patternToSoqlClause;
exports.picklistToSoqlClause = picklistToSoqlClause;
exports.ownershipClauses = ownershipClauses;
/** Convert a glob-style pattern like *AMER*PACE* to a SOQL LIKE string with % wildcards */
function globToSoqlLike(pattern) {
 ...
 */
function patternToSoqlClause(pattern, field) {
    ...
    if (positive.length === 1)
        clauses.push(positive[0]);
    else if (positive.length > 1)
        clauses.push(`(${positive.join(' OR ')})`);
 ...
 */
function picklistToSoqlClause(values, field) {
    if (!values)
        return '';
 ...
 */
function ownershipClauses(opts) {
    const { ownerRolePattern, ownerName, justMyData, currentUserId, ownerField = 'OwnerId', ownerNameField = 'Owner.Name', ownerRoleField = 'Owner.UserRole.Name', } = opts;
