...
const llmGateway = new openai_1.default({
...
async function callLLM(systemPrompt, userContent, onIO) {
...
async function classifyEventsBatched(events, systemPrompt, onIO, batchSize = 30) {
    const results = [];
    for (let i = 0; i < events.length; i += batchSize) {
        const batch = events.slice(i, i + batchSize);
        const userContent = JSON.stringify({ events: batch });
        const raw = await callLLM(systemPrompt, userContent, onIO ? (input, output) => onIO({ ...input, user: `[batch ${Math.floor(i / batchSize) + 1}] ${input.user}` }, output) : undefined);
        let parsed = {};
        try {
...
function matchesTerms(text, terms, requireInclude = false) {
    const lower = text.toLowerCase();
    if (terms.exclude.some(t => lower.includes(t)))
        return false;
    if (requireInclude && terms.include.length > 0)
        return terms.include.some(t => lower.includes(t));
    return true;
}
function fmt$(n) {
    if (!n)
...
function recencyScore(lastModifiedDate) {
    if (!lastModifiedDate)
        return 0;
...
function matchEventToAccounts(eventSummary, eventDescription, attendeeEmails, attendeeSfIds, accounts, // SF account records with OwnerId, Owner.Name etc
emailToSfId) {
    ...
    for (const acc of accounts) {
        ...
        if (accDomain && attendeeDomainsLower.some(d => d.startsWith(accDomain) || accDomain.startsWith(d.split('.')[0]))) {
            score += 30;
            signals.push('attendee domain match');
        }
        // Signal 4: attendee is SF user who owns the account or its opps
        if (attendeeSfIds.some(sfId => sfId === acc.OwnerId)) {
            score += 25;
            signals.push('attendee owns account');
        }
        if (score > 0)
            matches.push({ accountId: acc.Id, accountName: acc.Name, score, signals });
    }
    return matches.sort((a, b) => b.score - a.score);
}
// ── Opp ranking within an account ──────────────────────────────────────────────
function rankOpps(opps, myDCOppIds, teamDCOppIds, oppIdsWithMyActivity) {
    ...
    return opps
        ...
        if (o.ForecastCategoryName === 'Commit')
            reasons.push('Commit forecast');
        else if (o.ForecastCategoryName === 'Best Case')
            reasons.push('Best Case forecast');
        const amtScore = Math.min((o.Amount ?? 0) / 500_000, 1) * 20;
        score += amtScore;
        if (o.Amount >= 500_000)
            reasons.push(`${fmt$(o.Amount)} deal`);
        else if (o.Amount > 0)
            reasons.push(`${fmt$(o.Amount)}`);
        const rec = recencyScore(o.LastModifiedDate);
        score += rec * 15;
        if (rec >= 0.7)
            reasons.push('updated recently');
        if (myDCOppIds.has(o.Id)) {
            score += 20;
            reasons.push('your DC exists');
        }
        if (teamDCOppIds.has(o.Id)) {
            score += 25;
            reasons.push('team member has DC');
        }
        if (oppIdsWithMyActivity.has(o.Id)) {
            score += 10;
            reasons.push('activity logged');
        }
        const daysToClose = o.CloseDate
            ? Math.round((new Date(o.CloseDate).getTime() - Date.now()) / 86400000)
            : null;
        if (daysToClose !== null && daysToClose <= 30 && daysToClose >= 0)
...
// ── DC gap priority ─────────────────────────────────────────────────────────────
function dcPriority(opp, totalActivities, totalDurationMins) {
    const FC_RANK = { Commit: 3, 'Best Case': 2, Pipeline: 1 };
    ...
    const daysToClose = opp.CloseDate
        ...
        : 999;
    let priority = 'Low';
    if (fc >= 3 || daysToClose <= 30)
        priority = 'High';
    else if (fc >= 2 || daysToClose <= 90)
        priority = 'Medium';
    // Split % based on engagement
    let splitPct = 50;
    const totalHours = totalDurationMins / 60;
    if (totalActivities >= 7 || totalHours >= 24)
        splitPct = 100;
    else if (totalActivities <= 4 && totalHours < 4)
        splitPct = 25;
...
router.post('/briefing', async (req, res) => {
    ...
    try {
        ...
        const { currentUserId, dateFrom, dateTo, accountScope, // comma-separated name filter (include/exclude)
        ...
        if (!currentUserId) {
            send('error', { error: 'currentUserId required' });
            return res.end();
        }
        ...
        try {
            ...
            const calRaw = await new Promise((resolve, reject) => {
                ...
                proc.on('close', code => {
                    clearTimeout(t);
                    if (code !== 0)
                        return reject(new Error(`calendar claude exited ${code}: ${err.trim()}`));
                    resolve(out.trim());
                });
            ...
            rawCalEvents = items.filter((e) => e.status !== 'cancelled' && e.start?.dateTime);
        ...
        if (excludeTerms.exclude.length > 0) {
            ...
            if (dropped)
                send('debug', { message: `Excluded ${dropped} events by cal filter` });
        }
        ...
        try {
            ...
            const field = desc.fields.find((f) => f.name === 'SE_Task_Type__c');
            seTaskTypes = (field?.picklistValues ?? []).filter((v) => v.active).map((v) => v.value);
        ...
        calEvents.forEach(e => (e.attendees || []).forEach((a) => { if (a.email && !a.self)
            allEmails.add(a.email.toLowerCase()); }));
        const emailToSfId = new Map();
        const sfIdToUser = new Map();
        if (allEmails.size > 0) {
            try {
                ...
                ur.records.forEach((u) => {
                    if (u.Email) {
                        emailToSfId.set(u.Email.toLowerCase(), u.Id);
                        sfIdToUser.set(u.Id, u);
                    }
                });
            }
            catch { /* non-fatal */ }
        ...
        const sfEventsResult = await conn.query(`SELECT Id, Subject, StartDateTime, EndDateTime, WhatId, What.Name, AccountId, Account.Name
       FROM Event
       WHERE OwnerId = '${currentUserId}'
         AND StartDateTime >= ${fromDT}
         AND StartDateTime <= ${toDT}
       ORDER BY StartDateTime ASC LIMIT 300`);
        const sfLoggedEvents = sfEventsResult.records;
        const sfEventIndex = sfLoggedEvents.map((e) => {
        ...
        function isAlreadyLogged(calEvent) {
            ...
            return sfEventIndex.some(sf => {
                ...
                if (sf.utcDate !== calUtcDate)
                    return false;
                // Exact subject match
                if (sf.subject === subj)
                    return true;
                // Partial subject match: share first 8 chars in either direction
                return sf.subject.length >= 5 && (sf.subject.includes(subj.slice(0, 8)) || subj.includes(sf.subject.slice(0, 8)));
            });
        }
        // Duration of logged events per WhatId (opp) and AccountId
        const loggedDurationByOpp = new Map();
        const loggedCountByOpp = new Map();
        const loggedDurationByAccount = new Map();
        const loggedCountByAccount = new Map();
        const oppIdsWithMyActivity = new Set();
        sfLoggedEvents.forEach((e) => {
        ...
        const oppsResult = await conn.query(`SELECT Id, Name, Amount, StageName, ForecastCategoryName, CloseDate,
       ...
       ORDER BY LastModifiedDate DESC NULLS LAST LIMIT 1000`);
        let scopedOpps = oppsResult.records;
        // Helper: does an opp pass the scope name filters?
        function oppPassesScope(o) {
            const accName = (o['Account']?.Name ?? '').toLowerCase();
            const oppName = (o.Name ?? '').toLowerCase();
            if (scopeTerms.exclude.some(t => accName.includes(t) || oppName.includes(t)))
                return false;
            if (scopeTerms.include.length > 0) {
        ...
        if (attendeeSfIds.size > 0) {
            try {
                ...
                const attendeeOppRes = await conn.query(`SELECT Id, Name, Amount, StageName, ForecastCategoryName, CloseDate,
                  OwnerId, Owner.Name, Owner.Email, Owner.UserRole.Name,
                  AccountId, Account.Name, Account.OwnerId, Account.Owner.Name, Account.Owner.Email, Account.Owner.UserRole.Name,
                  LastModifiedDate, LastActivityDate
           FROM Opportunity
           ...
           ORDER BY LastModifiedDate DESC NULLS LAST LIMIT 200`);
                const existingIds = new Set(scopedOpps.map((o) => o.Id));
                const newOpps = attendeeOppRes.records
                ...
                if (calTokens.size > 0) {
                    ...
                    for (let i = 0; i < tokenList.length; i += TOKEN_BATCH) {
                        ...
                        try {
                            ...
                            calAccRecords.push(...res.records);
                        }
                        catch { /* non-fatal */ }
                    }
                    let added = 0;
                    calAccRecords.forEach((a) => {
                        if (!accountMap.has(a.Id)) {
                            const accNameLower = (a.Name ?? '').toLowerCase();
                            if (scopeTerms.exclude.some(t => accNameLower.includes(t)))
                                return;
                            if (scopeTerms.include.length > 0 && !scopeTerms.include.some(t => accNameLower.includes(t)))
                                return;
                    ...
                    if (added)
                        send('debug', { message: `Added ${added} account(s) from calendar event title matching` });
                }
            }
            catch { /* non-fatal */ }
        }
        ...
        const parentAccountMap = new Map(); // accountId → ultimate parent
        if (accounts.length > 0) {
            try {
                const ACC_BATCH = 200;
                const accDetailRecords = [];
                for (let i = 0; i < accounts.length; i += ACC_BATCH) {
                    const accIdList = accounts.slice(i, i + ACC_BATCH).map(a => `'${a.Id}'`).join(',');
                    const res = await conn.query(`SELECT Id, ParentId, Parent.Name, Parent.ParentId, Parent.Parent.Name, Parent.Parent.ParentId
             FROM Account WHERE Id IN (${accIdList})`);
                    accDetailRecords.push(...res.records);
                }
                const accDetailRes = { records: accDetailRecords };
                accDetailRes.records.forEach((a) => {
                ...
                if (missingNameIds.length > 0) {
                    ...
                    missingRes.records.forEach((a) => {
                        parentAccountMap.forEach((v, k) => { if (v.id === a.Id && !v.name)
                            v.name = a.Name; });
                    });
                }
            }
        ...
        if (supplementalAccountIds.length > 0) {
            try {
                ...
                const suppOppRes = await conn.query(`SELECT Id, Name, Amount, StageName, ForecastCategoryName, CloseDate,
                  OwnerId, Owner.Name, Owner.Email, Owner.UserRole.Name,
                  AccountId, Account.Name, Account.OwnerId, Account.Owner.Name, Account.Owner.Email, Account.Owner.UserRole.Name,
                  LastModifiedDate, LastActivityDate
           FROM Opportunity
           ...
           ORDER BY LastModifiedDate DESC NULLS LAST LIMIT 100`);
                const existingIds = new Set(scopedOpps.map((o) => o.Id));
                const newOpps = suppOppRes.records.filter(o => !existingIds.has(o.Id));
                if (newOpps.length) {
        ...
        const myDCOppIds = new Set();
        const teamDCsByOpp = new Map(); // oppId → [{seId, seName, role, splitPct}]
        if (scopedOpps.length > 0) {
            ...
            for (let i = 0; i < oppIds.length; i += DC_BATCH) {
                ...
                const dcRes = await conn.query(`SELECT Id, Opportunity__c, SE_Name__c, SE_Full_Name__c, Opportunity_Role__c, Split_Percentage__c
           FROM Deal_Contribution__c WHERE Opportunity__c IN (${batchIds})`);
                dcRes.records.forEach((d) => {
                    if (d.SE_Name__c === currentUserId) {
                        myDCOppIds.add(d.Opportunity__c);
                    }
                    else {
                        if (!teamDCsByOpp.has(d.Opportunity__c))
                            teamDCsByOpp.set(d.Opportunity__c, []);
        ...
        const teamDCOppIds = new Set(teamDCsByOpp.keys());
        // Subset where the DC holder was actually in one of your calendar meetings
        const attendeeTeamDCOppIds = new Set();
        teamDCsByOpp.forEach((dcs, oppId) => {
            if (dcs.some(d => attendeeSfIds.has(d.seId)))
                attendeeTeamDCOppIds.add(oppId);
        });
        ...
        const historicallyEngagedAccountIds = new Set();
        try {
            let histDCRecords = [];
            let histActRecords = [];
            try {
                const r = await conn.query(`SELECT Opportunity__r.AccountId FROM Deal_Contribution__c WHERE SE_Name__c = '${currentUserId}' AND CreatedDate >= ${historicalFromDT} AND Opportunity__r.AccountId != null`);
                histDCRecords = r.records;
            }
            catch { /* non-fatal */ }
            try {
                const r = await conn.query(`SELECT AccountId FROM Event WHERE OwnerId = '${currentUserId}' AND StartDateTime >= ${historicalFromDT} AND AccountId != null LIMIT 2000`);
                histActRecords = r.records;
            }
            catch { /* non-fatal */ }
            histDCRecords.forEach((d) => { const accId = d.Opportunity__r?.AccountId; if (accId)
                historicallyEngagedAccountIds.add(accId); });
            histActRecords.forEach((e) => { if (e.AccountId)
                historicallyEngagedAccountIds.add(e.AccountId); });
        ...
        const expandedEngagedAccountIds = new Set(historicallyEngagedAccountIds);
        ...
        const seIdToAccountIds = new Map();
        teamDCsByOpp.forEach((dcs, oppId) => {
            const opp = scopedOpps.find(o => o.Id === oppId);
            if (!opp?.AccountId)
                return;
            dcs.forEach(d => {
                if (!seIdToAccountIds.has(d.seId))
                    seIdToAccountIds.set(d.seId, new Set());
        ...
        const attendeeActsByAccount = new Map(); // accountId → Set<seId>
        if (attendeeSfIds.size > 0) {
            try {
                const idList = [...attendeeSfIds].map(id => `'${id}'`).join(',');
                const actRes = await conn.query(`SELECT AccountId, OwnerId FROM Event
           WHERE OwnerId IN (${idList})
             AND StartDateTime >= ${fromDT} AND StartDateTime <= ${toDT}
             AND AccountId != null LIMIT 500`);
                actRes.records.forEach((e) => {
                    if (!attendeeActsByAccount.has(e.AccountId))
                        attendeeActsByAccount.set(e.AccountId, new Set());
        ...
        const annotatedEvents = calEvents.map(e => {
            const attendeeEmails = (e.attendees || []).filter((a) => !a.self).map((a) => a.email?.toLowerCase() ?? '');
            const attendeeSfIdsForEvent = attendeeEmails.map((em) => emailToSfId.get(em)).filter(Boolean);
            ...
            const accountMatches = matchEventToAccounts(e.summary ?? '', e.description ?? '', attendeeEmails, attendeeSfIdsForEvent, accounts, emailToSfId);
            return {
                id: e.id ?? '',
                ...
                attendeeNames: (e.attendees || []).filter((a) => !a.self).map((a) => a.displayName || a.email || '').slice(0, 8),
        ...
        const suggestedTaskTypeMap = new Map();
        const suggestedGroupMap = new Map();
        if (unloggedAnnotated.length > 0) {
            ...
            try {
                ...
                function candidatesForEvent(e, limit = 8) {
                    const evAttendeeSfIds = e.attendeeSfIds;
                    if (e.matchScore >= 60) {
                    ...
                    const evDomains = new Set(e.attendeeEmails.map((em) => em.split('@')[1]?.split('.')[0] ?? '').filter(Boolean));
                    const evDomainsArr = [...evDomains];
...
Respond ONLY with valid JSON: {"events":[{"id":"string","taskType":"string_or_null","group":"string","accountI":number_or_null}]}`;
                const llmPayload = unloggedAnnotated.map(e => {
                    ...
                    return {
                        ...
                        attendeeDomains: [...new Set(e.attendeeEmails.map((em) => em.split('@')[1] ?? '').filter(Boolean))],
                ...
                llmResults.forEach((item) => {
                    ...
                    if (!ev || ev.matchScore >= 60)
                        return; // strong match already set — keep it
                    ...
                    if (prevAccountId && llmAccountId === prevAccountId) {
                        ...
                        if (prevAccountId) {
                            corrected++;
                            send('debug', { message: `LLM corrected "${ev.summary}" → ${accName}` });
                        }
                        else {
                            newMatch++;
                            send('debug', { message: `LLM matched "${ev.summary}" → ${accName}` });
                        }
                    }
                });
                ...
                const canonicalGroupByTitle = new Map();
                ...
                suggestedGroupMap.forEach((_, id) => {
                    ...
                    if (canon)
                        suggestedGroupMap.set(id, canon);
                });
        ...
        for (const accountId of allRelevantAccountIds) {
            ...
            const relatedAccountIds = new Set([accountId]);
            if (parentId)
                relatedAccountIds.add(parentId);
            parentAccountMap.forEach((p, aid) => { if (p.id === parentId && parentId)
                relatedAccountIds.add(aid); });
            // Also include opps the user has a DC on, even if Omitted (always show user's own DCs)
            const oppsForAccount = [
                ...new Map([...relatedAccountIds].flatMap(aid => oppsByAccount.get(aid) ?? [])
            ...
            const dcGaps = ranked
                ...
                if (oppIdsWithMyActivity.has(o.Id))
                    dcReasons.push(`you have ${loggedCountByOpp.get(o.Id) ?? 0} logged SF activit${(loggedCountByOpp.get(o.Id) ?? 0) !== 1 ? 'ies' : 'y'} against this opp`);
                if (o.ForecastCategoryName === 'Commit')
                    dcReasons.push('deal is in Commit');
                else if (o.ForecastCategoryName === 'Best Case')
                    dcReasons.push('deal is in Best Case');
                const daysToClose = o.CloseDate ? Math.round((new Date(o.CloseDate).getTime() - Date.now()) / 86400000) : null;
                if (daysToClose !== null && daysToClose >= 0 && daysToClose <= 30)
                    dcReasons.push(`closes in ${daysToClose} days`);
                const totalHours = Math.round(totalMins / 60 * 10) / 10;
                const splitReasonParts = [];
                if (totalActs >= 7 || totalHours >= 24)
                    splitReasonParts.push(`${totalActs} opp activit${totalActs !== 1 ? 'ies' : 'y'} / ${totalHours}h → 100%`);
                else if (totalActs <= 4 && totalHours < 4)
                    splitReasonParts.push(`${totalActs} opp activit${totalActs !== 1 ? 'ies' : 'y'} / ${totalHours}h → 25%`);
                else
                    splitReasonParts.push(`${totalActs} opp activit${totalActs !== 1 ? 'ies' : 'y'} / ${totalHours}h → 50%`);
                return {
                    oppId: o.Id,
                    oppName: o.Name,
                    accountId,
            ...
            const FC_SCORE = { Commit: 30, 'Best Case': 20, Pipeline: 10 };
            ...
            const groupScore = calStats.unloggedEvents.length * 20 +
            ...
            if (calStats.totalEvents > 0 || dcGaps.length > 0) {
                ...
                let mostEngagedAccountId = null;
                let mostEngagedAccountName = null;
                let topEngScore = -1;
                ...
                if (topEngScore <= 0) {
                    mostEngagedAccountId = null;
                    mostEngagedAccountName = null;
                }
                accountGroups.push({
                    accountId,
                    accountName: acc.Name,
        ...
        accountGroups.sort((a, b) => b.groupScore - a.groupScore);
...
router.post('/execute', async (req, res) => {
    try {
        const conn = (0, sf_1.getConnection)();
        const { currentUserId, activities, dcs } = req.body;
        if (!currentUserId)
        ...
        for (const dc of dcs ?? []) {
            try {
                const existing = await conn.query(`SELECT Id FROM Deal_Contribution__c WHERE SE_Name__c = '${currentUserId}' AND Opportunity__c = '${dc.oppId}' LIMIT 1`);
                const fields = { Opportunity_Role__c: dc.role, Split_Percentage__c: dc.splitPct, Comments__c: '#orbi' };
                if (existing.records.length > 0) {
                    await conn.sobject('Deal_Contribution__c').update({ Id: existing.records[0].Id, ...fields });
...
router.post('/chat', async (req, res) => {
    try {
        const conn = (0, sf_1.getConnection)();
        const { currentUserId, messages, briefingContext } = req.body;
        if (!currentUserId)
            return res.status(400).json({ error: 'currentUserId required' });
        ...
        const defaultRTId = rtRes.records[0]?.Id ?? null;
        const taskTypes = [...new Set(metaRes.records.map(r => r.SE_Task_Type__c).filter(Boolean))];
        ...
        if (candidateNames.length > 0) {
            ...
            const accLines = accRes.records.map(a => `- Account: ${a.Name} (id:${a.Id})`);
            const oppLines = oppRes.records.map(o => `- Opp: ${o.Name} [${o.StageName}, $${Math.round((o.Amount ?? 0) / 1000)}k, id:${o.Id}] on ${o.Account?.Name ?? 'unknown account'} (accountId:${o.AccountId})`);
            if (accLines.length || oppLines.length) {
                extraContext = `\nLive SF lookup results:\n${[...accLines, ...oppLines].join('\n')}`;
            }
        }
        const accountSummary = (briefingContext?.accounts ?? []).slice(0, 30).map(a => `- ${a.accountName} (id:${a.accountId}): ${a.opps.slice(0, 4).map(o => `${o.oppName} [$${Math.round((o.amount ?? 0) / 1000)}k, id:${o.oppId}]`).join('; ')}`).join('\n');
...
router.post('/chat/execute', async (req, res) => {
    try {
        const conn = (0, sf_1.getConnection)();
        const { currentUserId, plan } = req.body;
        if (!currentUserId)
            return res.status(400).json({ error: 'currentUserId required' });
        const results = [];
        ...
        for (const dc of plan.dcs ?? []) {
            try {
                const existing = await conn.query(`SELECT Id FROM Deal_Contribution__c WHERE SE_Name__c = '${currentUserId}' AND Opportunity__c = '${dc.oppId}' LIMIT 1`);
                const fields = { Opportunity_Role__c: dc.role, Split_Percentage__c: dc.splitPct, Comments__c: '#orbi-chat' };
                if (existing.records.length > 0) {
                    await conn.sobject('Deal_Contribution__c').update({ Id: existing.records[0].Id, ...fields });
                    results.push({ type: 'dc', name: dc.oppId, action: 'updated' });
                }
