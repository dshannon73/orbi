...
router.get('/', async (req, res) => {
    try {
        ...
        const { status, dateFrom, dateTo, account, amountMin, amountMax } = req.query;
        const clauses = [`OwnerId = '${userId}'`];
        ...
        if (dateFrom)
            clauses.push(`Oppty_Close_Date__c >= ${dateFrom}`);
        if (dateTo)
            clauses.push(`Oppty_Close_Date__c <= ${dateTo}`);
        if (account)
            clauses.push(`Oppty_Account__c LIKE '%${account.replace(/'/g, "\\'")}%'`);
        if (amountMin)
            clauses.push(`Oppty_Amount__c >= ${parseFloat(amountMin)}`);
        if (amountMax)
            clauses.push(`Oppty_Amount__c <= ${parseFloat(amountMax)}`);
        const result = await conn.query(`SELECT ${FIELDS} FROM Deal_Support_Request__c
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        if (!userId)
            return res.status(401).json({ error: 'Not authenticated' });
        const { statusComments, nextSteps } = req.body;
        const payload = { Id: req.params.id };
        if (statusComments !== undefined)
            payload.Status_Comments__c = statusComments;
        if (nextSteps !== undefined)
            payload.Next_Steps__c = nextSteps;
        const result = await conn.sobject('Deal_Support_Request__c').update(payload);
        res.json(result);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
router.post('/run-review', async (req, res) => {
    ...
    try {
        ...
        const dsrResult = await conn.query(`SELECT Id, Name, Status__c, Status_Detail__c, Oppty_Account__c, Opportunity__c,
       ...
       ORDER BY Oppty_Close_Date__c ASC NULLS LAST`);
        ...
        const activityByDsr = {};
        await Promise.all(dsrs.map(async (dsr) => {
            const oppId = dsr.Opportunity__c;
            if (!oppId)
                return;
            try {
                const acts = await conn.query(`SELECT Subject, ActivityDate, StartDateTime, Description, Type
           FROM ActivityHistory
           WHERE WhatId = '${oppId}' AND ActivityDate >= ${thirtyDaysAgo}
           ORDER BY ActivityDate DESC LIMIT 10`);
                activityByDsr[dsr.Id] = acts.records;
            }
            catch {
                activityByDsr[dsr.Id] = [];
            }
        }));
        // Build context block
        const fmt$ = (n) => n ? (n >= 1e6 ? `$${(n / 1e6).toFixed(1)}M` : `$${(n / 1000).toFixed(0)}K`) : '—';
        dsrContext = dsrs.map((dsr) => {
            const acts = activityByDsr[dsr.Id] ?? [];
            const actLines = acts.length
                ? acts.map((a) => `  - ${a.ActivityDate ?? a.StartDateTime?.slice(0, 10) ?? '?'}: ${a.Subject}${a.Description ? ` — ${a.Description.slice(0, 150)}` : ''}`).join('\n')
...
Start immediately with Phase 3 research. No preamble.`;
    const proc = (0, child_process_1.spawn)(CLAUDE_PATH, [
        '--print',
