...
function sfFiscalQuarter(date) {
    const m = date.getMonth();
    const y = date.getFullYear();
    if (m >= 1 && m <= 3)
        return { start: new Date(y, 1, 1, 0, 0, 0, 0), end: new Date(y, 3, 30, 23, 59, 59, 999) };
    if (m >= 4 && m <= 6)
        return { start: new Date(y, 4, 1, 0, 0, 0, 0), end: new Date(y, 6, 31, 23, 59, 59, 999) };
    if (m >= 7 && m <= 9)
        return { start: new Date(y, 7, 1, 0, 0, 0, 0), end: new Date(y, 9, 31, 23, 59, 59, 999) };
...
function dateRange(preset, customFrom, customTo) {
    const now = new Date();
    let start, end;
    switch (preset) {
        case 'today':
...
router.get('/events', async (req, res) => {
    const { datePreset = 'current_fq', dateFrom, dateTo, subject, attendee, description, currentUserId, } = req.query;
    ...
    try {
        ...
        function matchesField(value, filter) {
            ...
            if (exclude.some(t => v.includes(t)))
                return false;
            if (include.length && !include.some(t => v.includes(t)))
                return false;
            return true;
        }
        function matchesAttendee(attendees, filter) {
            const { include, exclude } = parseTerms(filter);
            const names = attendees.map(a => `${(a.displayName ?? '')} ${(a.email ?? '')}`.toLowerCase());
            if (exclude.some(t => names.some(n => n.includes(t))))
                return false;
            if (include.length && !include.some(t => names.some(n => n.includes(t))))
                return false;
            return true;
        }
        if (subject)
            events = events.filter(e => matchesField(e.summary ?? '', subject));
        if (attendee)
            events = events.filter(e => matchesAttendee(e.attendees ?? [], attendee));
        if (description)
            events = events.filter(e => matchesField(e.description ?? '', description));
        // Match against SF Events
        try {
            const conn = (0, sf_1.getConnection)();
            const ownerClause = currentUserId ? ` AND OwnerId = '${currentUserId}'` : '';
            const sfResult = await conn.query(`SELECT Id, Subject, StartDateTime FROM Event WHERE StartDateTime >= ${timeMin} AND StartDateTime <= ${timeMax} AND StartDateTime != null${ownerClause}`);
            const sfMap = new Map();
            ...
            events = events.map(e => {
                ...
                let sfId = sfMap.get(exactKey);
                if (!sfId) {
                    for (const [k, id] of sfMap.entries()) {
                        if (k.startsWith(day + '|')) {
                            const kSubj = k.split('|')[1];
                            if (kSubj.includes(subj) || subj.includes(kSubj)) {
                                sfId = id;
                                break;
                            }
                        }
