...
function buildPrompt(messages, userId, userName, userEmail) {
...
router.post('/run', (req, res) => {
    ...
    const proc = (0, child_process_1.spawn)(CLAUDE_PATH, ['--print', '--dangerously-skip-permissions', '--output-format', 'stream-json', '--verbose'], { env: CLAUDE_ENV });
    let killed = false;
    let stderrBuf = '';
    let stdoutBuf = '';
...
router.post('/auth', (req, res) => {
    ...
    res.setHeader('Connection', 'keep-alive');
    res.flushHeaders();
    const proc = (0, child_process_1.spawn)(MCP_ADAPTOR, args, {
    ...
    proc.stdout.on('data', (d) => emit('delta', d.toString()));
    proc.stderr.on('data', (d) => emit('delta', d.toString()));
    proc.on('close', (code) => {
        if (code !== 0)
